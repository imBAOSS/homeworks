...
View.prototype.exercise5 = function () {
  ...
  $square.on("click", event => {
    ...
    alert($clickEvent.attr("data-pos"));
  });
};
...
View.prototype.exercise6 = function () {
  ...
  let $square = $(".square");
  $square.each( (i) => {
    $($square[i]).css("background-color", window._randomColorString());
  });
};
...
View.prototype.exercise7 = function(){
  ...
  $square.on("mouseover", event => {
    let $hoverEvent = $(event.currentTarget);
    console.log($hoverEvent.css("background-color"));
  });
};
