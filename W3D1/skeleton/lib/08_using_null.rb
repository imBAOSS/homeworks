...
def teachers_and_depts
  ...
  execute(<<-SQL)
  SELECT
    teachers.name, COALESCE(depts.name, 'None') AS dept_name
  FROM
    teachers
    LEFT JOIN depts
      ON teachers.dept_id = depts.id
...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
  SELECT
    COUNT(teachers.name) AS number_of_teachers,
    COUNT(teachers.mobile) AS mobile
  FROM
    teachers
...
def dept_staff_counts
  ...
  execute(<<-SQL)
  SELECT
    depts.name, COUNT(teachers.name) AS num_staff
  FROM
    depts
    LEFT JOIN teachers
      ON teachers.dept_id = depts.id
