class UsersController < ApplicationController
  ...
  def search
    if params[:query].present?
      @users = User.where("username ~ ?", params[:query]).order("username asc")
      @exact_users = User.where("username ~ ?", "^" + params[:query]).order("username asc")
      @inexact_users = @users - @exact_users
      @users = @exact_users + @inexact_users
    else
      @users = User.none
    end
